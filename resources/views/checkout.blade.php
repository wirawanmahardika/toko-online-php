<x-layout>
    ...
    <div class="flex">
        <div class="basis-3/4 p-10 flex flex-col gap-y-4">
                    ...
                    @foreach ($items as $item)
                        <tr class="border-b-2 border-black text-left">
                            <td class="py-2 flex items-center gap-x-9">
                                <img src="{{asset('storage/'. $item['image'])}}" alt="baju" class="w-1/12">
                                <span>{{$item['name']}}</span>
                            </td>
            ...
            @if (count($items) === 0) <span class="italic text-xl font-semibold text-red-500 text-center">Belum ada item dikeranjang</span> @endif
